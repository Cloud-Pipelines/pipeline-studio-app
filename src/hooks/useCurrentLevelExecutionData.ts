...
export interface CachedExecutionData {
  executionId: string;
  details: GetExecutionInfoResponse;
  state: GetGraphExecutionStateResponse;
}

export const DEFAULT_TASK_STATUS = "WAITING_FOR_UPSTREAM";
export const PATH_DELIMITER = ".";
export const ROOT_PATH_START_INDEX = 1;

export const isAtRootLevel = (path: string[]) => path.length <= 1;

export const buildPathKey = (path: string[]) => path.join(PATH_DELIMITER);
 ...
 */
export const findExecutionIdAtPath = (
  path: string[],
  rootId: string,
 ...
 */
export const extractStatusFromStats = (
...
export const useCurrentLevelExecutionData = (
  ...
  rootState?: GetGraphExecutionStateResponse,
) => {
  ...
  const currentExecutionId = useMemo(() => {
    if (isAtRootLevel(currentSubgraphPath)) {
  ...
  }, [currentSubgraphPath, rootExecutionId, rootDetails]);

  // Fetch execution data for the current level
  // Note: At root level, this will fetch data that's already provided via props,
  // but having consistent fetch logic simplifies the code
  const {
    data: { details, state },
    isLoading,
    error,
  } = useFetchExecutionInfo(currentExecutionId, backendUrl, true);

  useEffect(() => {
    if (!details || !state || isAtRootLevel(currentSubgraphPath)) {
      return;
    }
  ...
  }, [details, state, currentExecutionId, currentSubgraphPath]);

  useEffect(() => {
    const isAtRoot = isAtRootLevel(currentSubgraphPath);
    const effectiveDetails = isAtRoot ? rootDetails || details : details;
    const effectiveState = isAtRoot ? rootState || state : state;

    const taskStatusMap = buildTaskStatusMap(effectiveDetails, effectiveState);

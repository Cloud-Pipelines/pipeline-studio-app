...
describe("summarizeTaskChange()", () => {
  ...
  test("counts changed execution settings", () => {
    const summary = summarizeTaskChange(
      taskDiff({
        settingDiffs: [
          { key: "retryStrategy", status: "changed" },
          { key: "isEnabled", status: "new" },
        ],
      }),
    );
    expect(summary).toBe("2 settings");
  });

  test("does not repeat a cache flip as a setting change", () => {
    const summary = summarizeTaskChange(
      taskDiff({
        cacheChanged: true,
        cacheDisabledB: true,
        settingDiffs: [{ key: "cachingStrategy", status: "changed" }],
      }),
    );
    expect(summary).toBe("cache disabled");
  });

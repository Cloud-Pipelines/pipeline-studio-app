import { isGraphImplementation, type ComponentSpec, type GraphSpec, type InputSpec, type OutputSpec, type TaskSpec } from "../componentSpec";
import type {
    Node,
  } from "@xyflow/react";
...
export const augmentComponentSpec = (
    ...
    const newInputs = componentSpec.inputs
      ?.map((inputSpec) => {
        ...
        const newAnnotationsWithPosition = includePositions ? {
          [NODE_LAYOUT_ANNOTATION_KEY]: inputPositionMap.get(
            inputSpec.name
          )
        } : {};

        const newAnnotations = { ...inputSpec.annotations, ...newAnnotationsWithPosition };

        const newInputSpecAnnotations = Object.keys(newAnnotations).length === 0 ? {} : { annotations: newAnnotations };
    ...
    const newOutputs = componentSpec.outputs
      ?.map((outputSpec) => {
        ...
        const newAnnotations = { ...outputSpec.annotations, ...newAnnotationsWithPosition };

        const newOutputSpecAnnotations = Object.keys(newAnnotations).length === 0 ? {} : { annotations: newAnnotations };
    ...
    const newTasks = Object.fromEntries(
      Object.entries(graphSpec.tasks || {})
        .map(([taskId, taskSpec]: [string, TaskSpec]) => {
          ...
          const newAnnotations = { ...taskSpec.annotations, ...newAnnotationsWithPosition };
          const newTaskSpecAnnotations = Object.keys(newAnnotations).length === 0 ? {} : { annotations: newAnnotations };
          const newTaskSpecComponentRef = !includeSpecs &&
            taskSpec.componentRef.spec !== undefined &&
            taskSpec.componentRef.url !== undefined ?
            {
              componentRef: {
                url: taskSpec.componentRef.url,
                spec: taskSpec.componentRef.spec,
                ...Object.fromEntries(
                  Object.entries(taskSpec.componentRef)
                    .filter(([key]) => key !== 'text' && key !== 'url' && key !== 'spec')
                )
              }
            } : {};

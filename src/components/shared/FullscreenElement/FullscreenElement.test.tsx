...
describe("FullscreenElement", () => {
  ...
  test("switches from normal to fullscreen mode", () => {
    ...
    expect(fullscreenContainer).toHaveStyle({ display: "contents" });
    expect(fullscreenContainer).toHaveStyle({ position: "" });
    ...
    expect(fullscreenContainer).toBeInTheDocument();
    expect(fullscreenContainer.parentElement).toBe(document.body);
    expect(fullscreenContainer).toHaveStyle({ position: "fixed" });
    expect(fullscreenContainer.style.zIndex).toBe("2147483647");
  });

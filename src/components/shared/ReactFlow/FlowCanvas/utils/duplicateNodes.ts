...
export const duplicateNodes = (
  ...
  console.log("Original Graph Outputs:", graphSpec.outputValues);
  ...
  Object.entries(newOutputs).forEach((output) => {
    ...
    if (!oldNodeId) {
      console.warn(`No old node ID found for new output ID: ${newNodeId}`);
      return;
    }

    const oldOutputId = nodeIdToOutputName(oldNodeId);

    console.log("old output id:", oldOutputId);
    console.log("new output id:", outputId);
    ...
    if (!outputValue) {
      console.warn(`No output value found for output ID: ${oldOutputId}`);
      return;
    }

    const updatedOutputValue = { ...outputValue };

    // if the outputvalue references a task that was also duplicated, we need to update it to refer to the duplicated task id
    if (typeof updatedOutputValue === "object" && updatedOutputValue !== null) {
      if ("taskOutput" in updatedOutputValue) {
        ...
        if (oldTaskNodeId in nodeIdMap) {
          ...
          updatedOutputValue.taskOutput.taskId = newTaskId;
        }
      }
    }
    ...
    console.log("outputValue:", updatedOutputValue);

    // todo: there is a bug where the original output will connect to a duplicated task as well as the new output
    // todo: implement connection modes for outputs
  });

  console.log("Updated Graph Outputs:", updatedGraphOutputs);
  ...
  if (config?.position) {
    ...
    newNodes.forEach((node) => {
      ...
      if (node.type === "task") {
        ...
        // todo: there is a bug where a new output node may jump to another location on the canvas after completing the duplication

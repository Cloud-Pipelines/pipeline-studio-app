...
const TaskNodeCard = ({
  ...
  const annotatedDimensions = JSON.parse(
    taskSpec.annotations?.["editor.position"] as string,
  ) as { x: string; y: string; width: string; height: string } | undefined;

  const dimensions = annotatedDimensions
    ? {
        w: parseInt(annotatedDimensions.width) || DEFAULT_DIMENSIONS.w,
        h: parseInt(annotatedDimensions.height) || DEFAULT_DIMENSIONS.h,
      }
    : DEFAULT_DIMENSIONS;
